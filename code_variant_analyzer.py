...
class VariantAnalyzer:
    ...
    def find_similar_variants(
        self, include_similar: bool = False
    ) -> List[VariantGroup]:
        ...
        for name, variants in self.variants.items():
            ...
            for variant in variants[1:]:
                ...
                if similarity >= self.thresholds["content_similarity"]:
                    ...
                    include_similar
                    and similarity >= self.thresholds["content_similarity"] * 0.8
                ):
                    similar.append(variant)
                    scores[variant.path] = similarity
                    diffs[variant.path] = self._generate_diff(base, variant)
    ...
    def _compute_similarity(self, a: CodeVariant, b: CodeVariant) -> float:
        ...
        weights = {"name": 0.2, "structure": 0.3, "content": 0.3, "calls": 0.2}
...
class VariantMerger:
    ...
    def merge_to_directory(
        self, output_dir: Path, preserve_structure: bool = True
    ) -> None:
        ...
        for group in self.groups:
            ...
            if preserve_structure:
                ...
                if self.format in ("python", "both"):
                    (output_dir / f"{group.base_variant.name}.py").write_text(
                        merged_source
                    )
                if self.format in ("markdown", "both"):
    ...
    def _merge_variant_group(self, group: VariantGroup) -> str:
        ...
        # Merge docstrings
        docstrings = [
            v.docstring for v in [base] + group.similar_variants if v.docstring
        ]
        merged_docstring = self._merge_docstrings(docstrings) if docstrings else None

        # Merge decorators
        all_decorators = {
            d for v in [base] + group.similar_variants for d in v.decorators
        }
...
class DashboardGenerator:
    ...
    def generate(
        ...
        html = template.render(entries=entries, diffs=diffs, show_all_variants=show_all)
            ...
            writer.writerow(
                {
                    ...
                    "score": (
                        max(group.similarity_scores.values())
...
def dashboard_command(args):
    ...
    generator.generate(
        args.summary_csv, args.diffs_dir, args.output, args.show_all_variants
    )
...
def main():
    ...
    compare_parser.add_argument(
        "--diffs-dir", type=Path, required=True, help="Output directory for diff files"
    )
    compare_parser.add_argument(
        "--summary-csv", type=Path, required=True, help="Output path for summary CSV"
    )
    compare_parser.add_argument(
        "--profile",
    ...
    dashboard_parser = subparsers.add_parser(
        "dashboard", help="Generate analysis dashboard"
    )
    dashboard_parser.add_argument(
        "--summary-csv", type=Path, required=True, help="Path to summary CSV file"
    )
    dashboard_parser.add_argument(
        "--diffs-dir", type=Path, required=True, help="Directory containing diff files"
    )
    dashboard_parser.add_argument(
        "--template-dir",
        type=Path,
    ...
    dashboard_parser.add_argument(
        "--output", type=Path, required=True, help="Output path for dashboard HTML"
    )
    dashboard_parser.add_argument(
        "--show-all-variants",
        action="store_true",
