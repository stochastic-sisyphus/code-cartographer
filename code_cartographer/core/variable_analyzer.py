        ...
        for target in node.targets:
            if isinstance(target, ast.Name):
                ...
                for definition in reversed(self.definitions):
                    if (
                        definition.name == target.id
                        and definition.line_number == target.lineno
                    ):
...
class VariableAnalyzer:
    ...
    def __init__(self, root: Path, exclude_patterns: List[str] = None):
        ...
        self.variable_flows: Dict[str, VariableFlow] = defaultdict(
            variable_flow_factory
        )

    def analyze_file(
        self, file_path: Path
    ) -> Tuple[List[VariableDefinition], List[VariableUsage]]:
