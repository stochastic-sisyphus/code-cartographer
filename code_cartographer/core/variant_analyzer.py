...
try:
    import numpy as np
    HAS_NUMPY = True
except Exception as e:  # pragma: no cover - allow running without numpy
    logging.getLogger(__name__).warning("numpy not installed: %s", e)
    np = None
    HAS_NUMPY = False
...
try:
    ...
    logging.getLogger(__name__).warning("nltk not installed: %s", e)
    HAS_NLP = False
...
try:
    ...
    logging.getLogger(__name__).warning("sentence-transformers not installed: %s", e)
        ...
        class NameStandardizer(ast.NodeTransformer):
            ...
            def visit_Name(self, node):
                if isinstance(node.ctx, ast.Store):
                    if node.id not in name_map:
                        name_map[node.id] = self._new_name("var_")
            ...
            def visit_FunctionDef(self, node):
                if node.name not in name_map:
                    name_map[node.name] = self._new_name("func_")
                node.name = name_map[node.name]
                return self.generic_visit(node)

            def visit_ClassDef(self, node):
                if node.name not in name_map:
                    name_map[node.name] = self._new_name("class_")
                node.name = name_map[node.name]
                return self.generic_visit(node)
...
class SemanticAnalyzer:
    ...
    def compare_blocks(self, block1: CodeBlock, block2: CodeBlock) -> float:
        ...
        """
        if not (self.model and HAS_NUMPY):
            return 0.0
    ...
    def find_variants(self, blocks: List[CodeBlock], threshold: float = 0.8) -> List[VariantGroup]:
        ...
        """
        if not (self.model and HAS_NUMPY):
            # Fallback to simple token Jaccard similarity
            groups = []
            used = set()
            for i, block1 in enumerate(blocks):
                ...
                if group:
                    used.add(i)
                    groups.append(VariantGroup(blocks=group, similarity=1.0))
            return groups
        ...
        if not (self.model and HAS_NUMPY):
            for i, block1 in enumerate(blocks):
                for block2 in blocks[i + 1:]:
                    ratio = difflib.SequenceMatcher(None, block1.normalized, block2.normalized).ratio()
                    if ratio >= threshold:
                        diff = list(
    ...
    def _cosine_similarity(v1, v2) -> float:
        """Calculate cosine similarity between two flat numeric iterables.

        Args:
            v1: Iterable of numbers (list, tuple, or 1D numpy array).
            v2: Iterable of numbers of the same length as ``v1``.
