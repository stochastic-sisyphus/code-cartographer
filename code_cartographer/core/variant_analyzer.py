...
class VariantAnalyzer:
    ...
    def _find_semantic_variants(self, blocks: List[CodeBlock]) -> List[VariantGroup]:
        """Find semantically similar code blocks."""
        variants = []

        # Encode all blocks
        encodings = self.encoder.encode([b.normalized for b in blocks])
        ...
        for i, block1 in enumerate(blocks):
            ...
            if group:
                variants.append(VariantGroup(blocks=group, similarity=similarity))
