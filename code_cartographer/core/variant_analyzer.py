...
class SemanticAnalyzer:
    ...
    def find_variants(self, blocks: List[CodeBlock], threshold: float = 0.8) -> List[VariantGroup]:
        ...
        if not self.model and not HAS_NLP:
            logger.warning("NLP dependencies not available, variant analysis will be limited")
    ...
    def _find_variants_token_based(self, blocks: List[CodeBlock], threshold: float = 0.8) -> List[VariantGroup]:
...
class VariantAnalyzer:
    ...
    def __init__(
        ...
        if HAS_TRANSFORMERS:
            ...
            logger.warning("Sentence transformers not available, semantic analysis will be limited")
    ...
    def _find_exact_duplicates(self, blocks: List[CodeBlock]) -> Dict[str, List[CodeBlock]]:
    ...
    def apply_merged_variants(self, backup: bool = True) -> Dict:
        ...
        if not HAS_TRANSFORMERS and not HAS_NLP:
            logger.warning("NLP dependencies not available, cannot apply merged variants")
