...
class DependencyNode:
    ...
    dependents: Set[str] = field(
        default_factory=set
    )  # Names of nodes that depend on this
...
class DependencyAnalyzer:
    ...
    def analyze(
        self, call_graph: Dict[str, List[str]], variable_flows: Dict[str, Any]
    ) -> Dict:
        """
        ...
        return {
            "nodes": {
                name: self._node_to_dict(node) for name, node in self.nodes.items()
            },
            "entry_points": [
                name for name, node in self.nodes.items() if node.is_entry_point
            ],
            ...
            while current_nodes:
                ...
                for node in current_nodes:
                    for dependent in self.nodes[node].dependents:
                        ...
                        if all(
                            dep in levels for dep in self.nodes[dependent].dependencies
                        ):
    ...
    def generate_dependency_graph(self, output_path: Path):
        ...
        # Add nodes
        for name, node in self.nodes.items():
            node_attrs = {"label": f"{name}\n({node.node_type})", "shape": "box"}
    ...
    def generate_sequential_order_graph(self, output_path: Path):
        ...
        for i, name in enumerate(init_order):
            ...
            dot.node(name, label=f"{i+1}. {name}\n({node.node_type})", shape="box")
