...
try:
    ...
    logging.getLogger(__name__).warning(
        "radon not installed, complexity metrics unavailable: %s", e
    )
    _HAS_METRICS = False
...
class CodeInspector(ast.NodeVisitor):
    ...
    def _detect_env_key(
        self,
        func: ast.AST | None,
        subscript: ast.Subscript | None,
        args: List[ast.AST] | None,
    ) -> str | None:
        """Return environment variable key if detected from call or subscript."""
        ...
        # os.environ["KEY"]
        if subscript and isinstance(subscript, ast.Subscript):
            s = subscript.slice
            key = None
            if isinstance(s, ast.Constant):
                key = s.value
            elif hasattr(s, "value") and isinstance(s.value, ast.Constant):
                key = s.value.value
    ...
    def visit_Call(self, node):
        func = node.func
        if key := self._detect_env_key(func, None, node.args):
            self.env_vars.add(key)
        self.api_calls |= self._detect_api_usage(func, list(node.args))

        self.generic_visit(node)

    def visit_Subscript(self, node):
        if key := self._detect_env_key(None, node, None):
            self.env_vars.add(key)
        self.generic_visit(node)
    ...
    def _extract_calls(self, node):
        ...
        for n in ast.walk(node):
            if isinstance(n, ast.Call):
                ...
                if key := self._detect_env_key(n.func, None, list(n.args)):
                    self.env_vars.add(key)
                self.api_calls |= self._detect_api_usage(n.func, list(n.args))

            elif isinstance(n, ast.Subscript):
                if key := self._detect_env_key(None, n, None):
                    self.env_vars.add(key)
        return calls
...
class ProjectAnalyzer:
    ...
    def _get_orphans(self) -> Dict[str, List[Dict[str, Any]]]:
        """Return orphaned functions, classes, and variables.

        Returns:
            Dict with keys ``"functions"``, ``"classes"``, and ``"variables"``.
            Each maps to a list of dictionaries containing ``name``, ``file``,
            and ``line`` keys for the orphaned element.
        """
