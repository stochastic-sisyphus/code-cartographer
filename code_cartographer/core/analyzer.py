...
class CodeInspector(ast.NodeVisitor):
    ...
    def visit_Call(self, node):
        func = node.func
        # Detect environment variable usage via os.getenv or os.environ.get
        ...
        # Detect API usage via popular HTTP libraries
        if isinstance(func, ast.Attribute) and isinstance(func.value, ast.Name):
            mod = func.value.id
            if mod in {"requests", "httpx", "aiohttp"}:
                self.api_calls.add(f"{mod}.{func.attr}")
        if isinstance(func, ast.Name) and func.id in {"urlopen", "urlretrieve"}:
            self.api_calls.add(func.id)

        for arg in node.args:
            if isinstance(arg, ast.Constant) and isinstance(arg.value, str):
                if arg.value.startswith("http://") or arg.value.startswith("https://"):
                    self.api_calls.add(arg.value)
    ...
    def visit_Subscript(self, node):
        if (
            ...
            elif hasattr(node.slice, "value") and isinstance(node.slice.value, ast.Constant):
    ...
    def visit_Constant(self, node):
        if isinstance(node.value, str) and (
            node.value.startswith("http://") or node.value.startswith("https://")
        ):
            self.api_calls.add(node.value)
        self.generic_visit(node)
        ...
        for n in ast.walk(node):
            if isinstance(n, ast.Call):
                if isinstance(n.func, ast.Attribute):
                    ...
                    if isinstance(n.func.value, ast.Name) and n.func.value.id in {"requests", "httpx", "aiohttp"}:
                        self.api_calls.add(f"{n.func.value.id}.{n.func.attr}")
                    ...
                    if n.func.id in {"urlopen", "urlretrieve"}:
                        self.api_calls.add(n.func.id)

             # Look for URL strings in call arguments
            if isinstance(n, ast.Call):
                for arg in n.args:
                    if isinstance(arg, ast.Constant) and isinstance(arg.value, str):
                        if arg.value.startswith("http://") or arg.value.startswith("https://"):
                ...
                if (
                    ...
                    if isinstance(n.slice, ast.Constant):
                        val = n.slice.value
                    elif hasattr(n.slice, "value") and isinstance(n.slice.value, ast.Constant):
                        val = n.slice.value.value
                    else:
                        val = None
                    if isinstance(val, str):
                        self.env_vars.add(val)
...
class ProjectAnalyzer:
    ...
    def _analyze_file(self, path: Path):
        ...
        file_meta = FileMetadata(
            ...
            api_calls=sorted(inspector.api_calls),
            env_vars=sorted(inspector.env_vars),
        )
