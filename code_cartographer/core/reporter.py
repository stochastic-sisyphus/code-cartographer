...
try:
    ...
    HAS_JINJA = True
except Exception as e:  # pragma: no cover - allow running without optional deps
    logging.getLogger(__name__).warning(
        "jinja2/markdown not installed, HTML report generation disabled: %s", e
    )
    Environment = None
...
class ReportGenerator:
    ...
    def generate_markdown_report(self, analysis_data: Dict, 
        ...
        if "dependency_levels" in analysis_data:
            ...
            if "initialization_order" in analysis_data:
                ...
                if len(analysis_data["initialization_order"]) > 20:
                    report_lines.append(f"... and {len(analysis_data['initialization_order']) - 20} more")
            ...
            if "cycles" in analysis_data and analysis_data["cycles"]:
                ...
                report_lines.append("The following cycles in dependencies were detected:\n")
                
                for i, cycle in enumerate(analysis_data["cycles"][:10]):
                    report_lines.append(f"Cycle {i+1}: " + " → ".join(cycle) + " → " + cycle[0])
                
                if len(analysis_data["cycles"]) > 10:
                    report_lines.append(f"... and {len(analysis_data['cycles']) - 10} more cycles")
        ...
        for file_data in analysis_data.get("files", []):
            ...
            if file_data.get("api_calls"):
                report_lines.append("\n#### API Calls")
                for api in file_data.get("api_calls", []):
                    report_lines.append(f"- {api}")

            if file_data.get("env_vars"):
                report_lines.append("\n#### Environment Variables")
                for var in file_data.get("env_vars", []):
            ...
            for definition in file_data.get("definitions", []):
                ...
                report_lines.append(f"- **{name}** ({category}, {line_count} lines){indicator_str}")
                ...
                if definition.get("inbound_calls"):
                    ...
                    if len(callers) <= 5:
                        ...
                        report_lines.append(f"  - Called by: {', '.join(callers[:5])} and {len(callers) - 5} more")
                ...
                if definition.get("prerequisites"):
                    ...
                    if len(prereqs) <= 5:
                        ...
                        report_lines.append(f"  - Prerequisites: {', '.join(prereqs[:5])} and {len(prereqs) - 5} more")
        ...
        if "variants" in analysis_data and analysis_data["variants"]:
            ...
            for name, variant_data in analysis_data["variants"].items():
                ...
                report_lines.append(f"Found {len(implementations)} similar implementations:\n")
                
                for impl in implementations:
                    report_lines.append(f"- In file: {impl.get('path', 'unknown')}")
                
                report_lines.append("\n**Recommendation**: " + variant_data.get("refactor_prompt", "Consider refactoring these implementations.").split("\n")[0])
                report_lines.append("\n")
    ...
    def generate_html_report(self, analysis_data: Dict, markdown_path: Path,
                           output_path: Optional[Path] = None) -> Path:
        """
        ...
        if markdown is None:
            warning_message = (
                "# WARNING: HTML report could not be generated\n"
                "The required 'markdown' package is not installed. "
                "This file contains the raw Markdown content instead of an HTML report.\n\n"
            )
            with open(output_path, "w") as f:
...
{% endfor %}</pre>
    ...
    def _prepare_orphan_data(self, analysis_data: Dict) -> Dict:
        ...
        return analysis_data.get("orphans", {
            "functions": [],
            "classes": [],
            "variables": []
        })
