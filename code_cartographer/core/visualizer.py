...
class CodeVisualizer:
    ...
    def generate_function_call_graph(
        self, call_graph: Dict[str, List[str]], output_path: Optional[Path] = None
    ) -> Path:
        """
    ...
    def generate_class_hierarchy(
        self, class_data: Dict[str, Dict], output_path: Optional[Path] = None
    ) -> Path:
        """
        ...
        # Draw the graph
        nx.draw_networkx_nodes(
            G, pos, node_size=2000, node_color="lightblue", alpha=0.8
        )

        nx.draw_networkx_edges(
            G, pos, width=1.5, alpha=0.7, edge_color="gray", arrows=True, arrowsize=15
        )
    ...
    def generate_dependency_heatmap(
        self,
        dependency_matrix: Dict[str, Dict[str, int]],
    ...
    def generate_variable_usage_chart(
        self, variable_data: Dict[str, Dict], output_path: Optional[Path] = None
    ) -> Path:
        """
    ...
    def generate_orphan_analysis(
        self, orphans: Dict[str, List[Dict]], output_path: Optional[Path] = None
    ) -> Path:
        """
    ...
    def generate_prerequisite_graph(
        self, prerequisites: Dict[str, Set[str]], output_path: Optional[Path] = None
    ) -> Path:
        """
        ...
        nx.draw_networkx_edges(
            G, pos, width=1.0, alpha=0.6, edge_color="gray", arrows=True, arrowsize=10
        )
    ...
    def generate_initialization_sequence(
        self, sequence: List[str], output_path: Optional[Path] = None
    ) -> Path:
        """
        ...
        # Draw the graph
        nx.draw_networkx_nodes(
            G, pos, node_size=1500, node_color="lightgreen", alpha=0.8
        )

        nx.draw_networkx_edges(
            G, pos, width=2.0, alpha=0.7, edge_color="gray", arrows=True, arrowsize=15
        )
    ...
    def _prepare_complexity_data(self, analysis_data: Dict) -> Dict:
        ...
        for file_data in analysis_data.get("files", []):
            ...
            lines = sum(
                d.get("line_count", 0) for d in file_data.get("definitions", [])
            )

            if complexity and complexity > 10:
                high_complexity.append(
                    {"name": file_name, "complexity": complexity, "lines": lines}
                )

            chart_data["datasets"][0]["data"].append(
                {"x": lines, "y": complexity, "r": 5 + (complexity / 5)}
            )
